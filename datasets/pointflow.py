...
class Uniform15KPC(Dataset):
    def __init__(
        ...
        if (
            ...
            self.all_points_std = (
                np.amax(
                    (
                        (np.amax(self.all_points, axis=1)).reshape(
                            B, 1, input_dim
                        )
                        - (np.amin(self.all_points, axis=1)).reshape(
                            B, 1, input_dim
                        )
                    ),
                    axis=-1,
        ...
        if self.disable_normalization:
            self.all_points_mean = np.zeros_like(self.all_points_mean)
            self.all_points_std = np.ones_like(self.all_points_std)
        ...
        self.all_points_mean_aux = self.all_points_mean
        self.all_points_std_aux = self.all_points_std

        self.get_all_points_statistics()
    ...
    def get_pc_stats(self, idx):
        ...
        return (
            self.all_points_mean.reshape(1, -1),
            self.all_points_std.reshape(1, -1),
        )

    def get_all_points_statistics(self):
        B = self.all_points.shape[0]
        self.all_points_mean_aux = (
            (np.amax(self.all_points, axis=1)).reshape(B, 1, self.input_dim)
            + (np.amin(self.all_points, axis=1)).reshape(B, 1, self.input_dim)
        ) / 2
        self.all_points_std_aux = (
            np.amax(
                (
                    (np.amax(self.all_points, axis=1)).reshape(
                        B, 1, self.input_dim
    ...
    def normalize_given_points(
        self, points: torch.Tensor, indices: torch.Tensor
    ) -> torch.Tensor:
        indices_np = Uniform15KPC.to_numpy(indices)
        means = torch.as_tensor(
            self.all_points_mean_aux[indices_np],
            device=points.device,
            dtype=points.dtype,
        )
        std = torch.as_tensor(
            self.all_points_std_aux[indices_np],
            device=points.device,
            dtype=points.dtype,
        )

        return (points - means) / std

    @classmethod
    def normalize_generated_points(cls, points: torch.Tensor) -> torch.Tensor:
        points_max = points.max(dim=1, keepdim=True)[0]
        points_min = points.min(dim=1, keepdim=True)[0]

        mean = (points_max + points_min) / 2
        std = (points_max - points_min).max(dim=-1, keepdim=True)[0] / 2
    ...
    def renormalize(self, mean, std):
        ...
        if self.disable_normalization:
            self.all_points_mean = np.zeros_like(mean)
            self.all_points_std = np.ones_like(std)
        else:
