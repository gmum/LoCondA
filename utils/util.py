...
def find_latest_epoch(dirpath):
    # Files with weights are in format ddddd_{D,E,G,A}.pth
    epoch_regex = re.compile(r'^(?P<n_epoch>\d+)_[ADEG]\.pth$')
    epochs_completed = []
...
def get_weights_dir(config, experiment=None):
    if experiment is None:
        experiment = 'training'
    if config.get('weights_path'):
        weights_dir = config['weights_path']
    else:
        weights_dir = join(config['results_root'], config['arch'], experiment, get_distribution_dir(config),
